...
7.  **Out-of-Scope Handling**: If asked about topics completely unrelated to LWWD or the provided documents (such as viruses, general world trivia, unrelated dates, etc.), respond: "I am sorry, but I am here specifically to help you with Leucadia-related knowledge and District operations. I would be happy to assist you with any questions about our work here! 🌊"
...
async def seed_prompts():
    ...
    async with engine.begin() as conn:
        print("Seeding AI Configs...")
        
        # Check and Insert Internal
        result = await conn.execute(text("SELECT id FROM ai_config WHERE agent_type = 'internal'"))
        if not result.scalar():
            print("Inserting Internal Prompt...")
            await conn.execute(
                text("INSERT INTO ai_config (agent_type, system_prompt, model, temperature, created_at, updated_at) VALUES (:agent, :prompt, 'gpt-4o-mini', 0.7, NOW(), NOW())"),
                {"agent": "internal", "prompt": INTERNAL_PROMPT} # Fixed generic params
            )
        else:
            print("Internal Prompt already exists.")

        # Check and Insert External
        result = await conn.execute(text("SELECT id FROM ai_config WHERE agent_type = 'external'"))
        if not result.scalar():
            print("Inserting External Prompt...")
            await conn.execute(
                text("INSERT INTO ai_config (agent_type, system_prompt, model, temperature, created_at, updated_at) VALUES (:agent, :prompt, 'gpt-4o-mini', 0.7, NOW(), NOW())"),
                {"agent": "external", "prompt": EXTERNAL_PROMPT}
            )
        else:
            print("External Prompt already exists.")
