...
import { CRM_INTEGRATIONS } from "@/lib/integrations-data";
                ...
                <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                    <div className="max-w-[1400px] mx-auto space-y-6 pb-12">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            <div>
                                <h1 className="text-xl font-bold text-slate-800 tracking-tight uppercase font-display">CRM Integrations</h1>
                                <p className="text-xs text-slate-500 mt-0.5 font-medium font-sans">Power Internal Assistant with your customer relationship data.</p>
                            </div>
                                ...
                                {CRM_INTEGRATIONS.map((crm) => (
                                    ...
                                    >
                                        ...
                                        <div
                                            ...
                                            style={{ backgroundColor: crm.primary_color }}
                                        ></div>
                                        ...
                                        <div className="absolute top-0 right-0 p-6 opacity-[0.03] group-hover:opacity-[0.07] transition-opacity duration-500 translate-x-4 -translate-y-4">
                                            <img src={crm.logo_url} alt="" className="w-32 h-32 grayscale object-contain" onError={(e) => e.currentTarget.style.display = 'none'} />
                                        </div>
                                        ...
                                        <div className="relative z-10 flex-1">
                                            <div
                                                ...
                                                style={{
                                                    borderColor: `${crm.primary_color}20`
                                                }}
                                            >
                                                <img src={crm.logo_url} alt={crm.name} className="w-full h-full object-contain" onError={(e) => e.currentTarget.style.display = 'none'} />
                                            </div>
                                            <h3 className="text-base font-bold text-slate-800 mb-2 tracking-tight">{crm.name}</h3>
                                            <p className="text-slate-500 text-xs leading-relaxed mb-6 line-clamp-3 group-hover:text-slate-600 transition-colors">{crm.description}</p>
                                        </div>
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
                                <div className="space-y-2">
                                    <h5 className="font-bold text-xs text-slate-800 uppercase tracking-wider">End-to-End Encryption</h5>
                                    <p className="text-[11px] text-slate-500 leading-relaxed">All CRM data is encrypted at rest and during transit using AES-256 and TLS 1.3 standards.</p>
                                </div>
                                <div className="space-y-2">
                                    <h5 className="font-bold text-xs text-slate-800 uppercase tracking-wider">Scoped Permissions</h5>
                                    <p className="text-[11px] text-slate-500 leading-relaxed">Leucadia only requests read-only scopes necessary for document and record indexing.</p>
                                </div>
                                <div className="space-y-2">
                                    <h5 className="font-bold text-xs text-slate-800 uppercase tracking-wider">User Data Control</h5>
                                    <p className="text-[11px] text-slate-500 leading-relaxed">You can revoke access and delete synced CRM data directly from your integration settings at any time.</p>
                                </div>
                            </div>
                        </div>
                    </div>
